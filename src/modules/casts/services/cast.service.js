...
class CastService {
    ...
    getCastById = async(id) => {
        const cast = await Cast.findById(id);
        if (!cast) {
            throw new AppError("Cast member was not found", 404)
        }
        return cast; // إرجاع المتغير مباشرة بدلاً من استدعاء قاعدة البيانات مرة أخرى
    };
    ...
    updateCast = async(id, data) => {
        ...
        if (data.name) {
            // يجب استثناء الممثل الحالي من فحص تكرار الاسم في حال لم يغير اسمه
            const existingCast = await Cast.findOne({
                name: {
                    $regex: `^${data.name}$`,
                    $options: "i"
            ...
            if (existingCast) {
                throw new AppError("Cast member name already exists", 400);
            }
        }
        return await Cast.findByIdAndUpdate(id, data, {
            new: true,
            runValidators: true // تطبيق شروط الموديل
        });
    };
    ...
    deleteCast = async(id) => {
        ...
        // بتشغيل الميدل وير لتنظيف جداول الربط
        return await Cast.findByIdAndDelete(id);
    };
