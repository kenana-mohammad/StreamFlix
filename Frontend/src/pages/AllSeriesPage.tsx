...
export default function AllSeriesPage() {
  ...
  const loadData = async () => {
    try {
      ...
      // جلب المسلسلات من endpoint محدد (أفضل من generic content)
      const seriesData = await apiClient.getAllSeries();
      const normalizedSeries = normalizeContentArray(Array.isArray(seriesData) ? seriesData : (seriesData || []));
      setAllContent(normalizedSeries);
      ...
      const msg = err?.response?.data?.message || 'Failed to load series';
      setError(msg);
      toast.error(msg);
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-ink-950 flex flex-col">
      ...
      <main className="flex-grow pt-24 pb-12">
        <div className="px-4 sm:px-6 lg:px-12 max-w-7xl mx-auto w-full">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-4xl font-bold text-white mb-2">All Series</h1>
            <p className="text-gray-400">Browse all available series</p>
          </div>
          ...
          {filteredContent.length > 0 ? (
            <>
              ...
              <div className="text-center text-gray-400">
                Showing {filteredContent.length} series{filteredContent.length !== 1 ? '' : ''}
              </div>
            </>
          ) : (
